...
import { AuthService } from '../../core/services/client-services/auth-service';
import { AccountService } from '../../core/services/client-services/account-service';
...
export class LoginSignupComponent {
  ...
  // protected
  protected readonly footerExtraPages = footerExtraPages;

  // signals
  protected isShowPassword = signal(false);
  protected currPage = signal<PageTypes>('signin');

  // computed
  private serverError = computed(() => {
    if (this.currPage() == 'signin') return this.authService.loginServerError();

    return this.accountService.registerServerError();
  });
  ...
  getAllErrors(): string[] {
    ...
    if (this.serverError()) {
      errors = [...errors, this.serverError()];
    }
